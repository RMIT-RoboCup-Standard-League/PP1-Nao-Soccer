...

{
    ...
    for (i = 0; i < Joints::NUMBER_OF_JOINTS; ++i) {
        ...
        current_.angles[i] = 0.0f;
        current_.stiffnesses[i] = 0.0f;
        current_.temperatures[i] = 0.0f;
        last_command_.angles[i] = 0.0f;
    }
...
void SimulationThread::tick()
{
    ...
    current_ = s.joints; // Store current joint angles (in radians)
    ...
    JointValues& joints = shared_data_->joints[shared_data_->actuators_read];
    ...
    const float lle1_request = joints.angles[8];
    for (int i = 0; i < Joints::NUMBER_OF_JOINTS; ++i)
    {
        ...
        // move approx 1.22173 * velocity.
        //
        // TODO @future consider dynamic VEL_PER_TICK variable
        float prediction = current_.angles[i] + (VEL_PER_TICK * last_command_.angles[i]);
        ...
        float difference = joints.angles[i] - prediction;
        ...
        //
        float velocity =  difference ? (difference / VEL_PER_TICK) : 0;
        ...
        joints.angles[i] = velocity;
    }

    // Build effector command for server
    EffectorCommand cmd;
    if (cmd.joints.fromJointValues(joints))
    {
        ...
        float prediction = DEG2RAD(recv.joints.rlj1) + (VEL_PER_TICK * prev_rlj1_);
    ...
    last_command_ = joints;

    // rUNSWift is on a 10ms tick but simulation server is 20ms
    // NOTE We don't need to sleep because we block on the server anyway
}
