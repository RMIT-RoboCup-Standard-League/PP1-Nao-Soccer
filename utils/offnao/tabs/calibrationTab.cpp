...
#include "perception/vision/Vision.cpp"
...
void CalibrationTab::drawOverlays(QImage *image) {
      ...
      case oSELECTED_UNCLASSIFIED:
         for (int i = 0; i < IMAGE_COLS; ++i) {
            for (int j = 0; j < IMAGE_ROWS; ++j) {
               ...
               u = getu(currentFrame, j, i, C);
               v = getv(currentFrame, j, i, C);
               if (getCurrentClassifier()->getClassifiedColour(y, u, v) == cBACKGROUND ||
   ...
   if (checkVisionOverlay->isChecked()) {
      ...
      uint8_t *ptr = reinterpret_cast<uint8_t*>(getCurrentClassifier()->getNnmcPointer());

      for (int i = 0; i < 2097152; i++) {
         ((GreenYUVClassifier *) vision->colour_classifier_top_)->nnmc_[i] = ptr[i];
         ((GreenYUVClassifier *) vision->colour_classifier_bot_)->nnmc_[i] = ptr[i];
      }
      ...
      for (p = ((FieldBoundaryFinder *) vision->getMiddleInfoProcessor_(MID_PROCESSOR_FIELD_BOUNDARY))->boundaryPointsTop.begin();
           p != ((FieldBoundaryFinder *) vision->getMiddleInfoProcessor_(MID_PROCESSOR_FIELD_BOUNDARY))->boundaryPointsTop.end(); ++p) {
         painter.drawEllipse(QPoint((*p).x(), (*p).y()), 2, 2);
      }
      for (p = ((FieldBoundaryFinder *) vision->getMiddleInfoProcessor_(MID_PROCESSOR_FIELD_BOUNDARY))->boundaryPointsBot.begin();
           p != ((FieldBoundaryFinder *) vision->getMiddleInfoProcessor_(MID_PROCESSOR_FIELD_BOUNDARY))->boundaryPointsBot.end(); ++p) {
         painter.drawEllipse(QPoint((*p).x(), (*p).y()), 2, 2);
      }
      
      std::vector<FieldBoundaryInfo> fieldBoundaries = info_out.boundaries;

      std::vector<FieldBoundaryInfo>::const_iterator i;
      for (i = fieldBoundaries.begin (); i != fieldBoundaries.end (); ++ i) {

         const RANSACLine &line = i->imageBoundary;
         painter.setPen(QColor(255, 211, 9));
         painter.drawLine(
            0,
