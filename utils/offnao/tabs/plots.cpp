...
AngleXPlot::AngleXPlot(QWidget *parent)
   : DataPlot(parent) {
   for (int i = 0; i < PLOT_SIZE; ++i)
...
void AngleXPlot::push(SensorValues sensors, bool left) {
   if (left)
...
//using it for gyroX
GyroXPlot::GyroXPlot(QWidget *parent)
   : DataPlot(parent) {
   for (int i = 0; i < PLOT_SIZE; ++i) {
...
void GyroXPlot::push(SensorValues sensors, bool left) {
   if (left)
      for (int i = PLOT_SIZE-1; i > 0; --i){
         data[i] = data[i-1];
         data2[i] = data2[i-1];
...
GyroYPlot::GyroYPlot(QWidget *parent)
   : DataPlot(parent) {
   for (int i = 0; i < PLOT_SIZE; ++i) {
      data[i] = 0.0; data2[i] = 0.0;
   }
...
void GyroYPlot::push(SensorValues sensors, bool left) {
   if (left)
      for (int i = PLOT_SIZE-1; i > 0; --i){
         data[i] = data[i-1];
         data2[i] = data2[i-1];
