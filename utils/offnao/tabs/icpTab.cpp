...
void ICPTab::drawICP() {
   if (blackboard && !isnan(fixedPos.theta()) && !isnan(fixedPos.x()) && !isnan(fixedPos.y())) {
      ...
      int icpResult;
      if (useTeamBall) {
         icpResult = ICP::localise(fixedPos, fieldFeatures, posts, awayGoalProb, headYaw, fieldBoundaries, ballRRC, isLost, teamBall);
         fieldView.redraw(blackboard, fixedPos, icpResult, ICP::getCombinedObs(), ballRRC, teamBall);
      } else {
         icpResult = ICP::localise(fixedPos, fieldFeatures, posts, awayGoalProb, headYaw, fieldBoundaries, ballRRC, isLost);
...
*/
      ...
      icpStream << "Number of field boundaries    : " << fieldBoundaries.size() << std::endl;
