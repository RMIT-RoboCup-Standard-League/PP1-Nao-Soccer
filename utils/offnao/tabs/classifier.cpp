...
void Classifier::getUndoStatus(int& levels, Colour& lastColour) {
   ...
   if (not undoBuffer.empty()) {
      if (undoBuffer.back().gaussians.empty()) {
         lastColour = cBACKGROUND; // Hack just to not break offnao
      } else {
...
Colour Classifier::maxWeightIndex(Weights &weights) {
   ...
   int maxIndex = cBACKGROUND; // Hack from being cUNCLASSIFIED just to avoid breaking offnao
...
Colour Classifier::getClassifiedColour(int y, int u, int v) {
   ...
   if (y < 0 or y/2 >= YMAX or
         u < 0 or u/2 >= UMAX or
         v < 0 or v/2 >= VMAX) {
      return cBACKGROUND; // Hack from being cUNCLASSIFIED just to avoid breaking offnao
   }
